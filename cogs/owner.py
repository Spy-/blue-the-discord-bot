...
def set_cog(cog, value):
    data = dataIO.load_json("data/blue/cogs.json")
    data[cog] = value
    dataIO.save_json("data/blue/cogs.json", data)
...
class Owner:
    ...
    def __init__(self, bot):
        ...
        self.disabled_commands = dataIO.load_json(
            "data/blue/disabled_commands.json")
        self.global_ignores = dataIO.load_json("data/blue/global_ignores.json")
    ...
    async def owner(self, ctx):
        ...
        if self.bot.settings.no_prompt is True:
            await self.bot.say("Console interaction is disabled. Start Bot "
    ...
    async def stream(self, ctx, streamer=None, *, stream_title=None):
        ...
        if stream_title:
            ...
            log.debug('Owner has set streaming status and url to "{}" and {}'.format(
                stream_title, streamer))
    ...
    async def _blacklist_add(self, user: discord.Member):
        """Adds user to Bot's global blacklist"""
    ...
    async def _blacklist_remove(self, user: discord.Member):
        """Removes user from Bot's global blacklist"""
    ...
    async def _whitelist_add(self, user: discord.Member):
        """Adds user to Bot's global whitelist"""
    ...
    async def _whitelist_remove(self, user: discord.Member):
        """Removes user from Bot's global whitelist"""
    ...
    async def restart(self, silently: bool=False):
        ...
        Makes Bot quit with exit code 26
 ...
 '''
    ...
    def save_global_ignores(self):
        dataIO.save_json("data/blue/global_ignores.json", self.global_ignores)

    def save_disabled_commands(self):
        dataIO.save_json("data/blue/disabled_commands.json",
...
def check_files():
    if not os.path.isfile("data/blue/disabled_commands.json"):
        print("Creating empty disabled_commands.json...")
        dataIO.save_json("data/blue/disabled_commands.json", [])

    if not os.path.isfile("data/blue/global_ignores.json"):
        ...
        dataIO.save_json("data/blue/global_ignores.json", data)
