...
class Bot(commands.Bot):
    ...
    def add_message_modifier(self, func):
        if not callable(func):
            raise TypeError(
                "The message modifier function must be a callable.")
    ...
    def remove_message_modifier(self, func):
        if func not in self._message_modifiers:
            raise RuntimeError(
                "Function not present in the message modifiers.")
...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    bot = bot_class(formatter=formatter,
                    description="All the commands for the bot of the Castillon Server", pm_help=None)
    ...
    async def set_bot_owner():
        ...
        if bot.settings.owner:
            owner = discord.utils.get(
                bot.get_all_members(), id=bot.settings.owner)
            if not owner:
                try:
...
def check_folders():
    folders = ("data", "data/blue", "cogs", "cogs/utils")
...
def set_logger(bot):
    ...
    fhandler = logging.handlers.RotatingFileHandler(
        filename='data/blue/blue.log', encoding='utf-8', mode='a',
    ...
    handler = logging.FileHandler(
        filename='data/blue/discord.log', encoding='utf-8', mode='a')
...
def load_cogs(bot):
    ...
    try:
        registry = dataIO.load_json("data/blue/cogs.json")
    except:
        registry = {}
    ...
    if bot.settings._no_cogs:
        ...
        if not os.path.isfile("data/blue/cogs.json"):
            dataIO.save_json("data/blue/cogs.json", {})
        return
    ...
    dataIO.save_json("data/blue/cogs.json", registry)
